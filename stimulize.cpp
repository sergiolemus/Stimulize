...
void printCommentsAndStep( const std::vector<std::string> & inputs, const std::vector<char> & input_values, int v_nodes )
{
    ...
    for ( int i = 0; i < v_nodes; i++ )
    {
        std::cout << static_cast<char>( input_values[ i ] + 0x30 )
                    << ( ( i != v_nodes - 1 ) ? ", " : " )\ns\n\n" );
    }
    std::cout << std::flush;
    return;
                      ...
                      int t_index, int t_nodes, bool t, int s_index )
{
    ...
    if( !t )
    {
        ...
        printCommentsAndStep( inputs, input_values, v_nodes );
